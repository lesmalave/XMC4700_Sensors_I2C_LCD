...
const int calibrationLed = 25;                      //when the calibration start , LED pin 13 will light up , off when finish calibrating
const int MQ7_PIN = A1;                                //define which analog input channel you are going to use
...
{ 
  ...
  Ro = MQCalibration(MQ7_PIN);                         //Calibrating the sensor. Please make sure the sensor is in clean air         
...
{  
  ...
  iPPM_LPG = MQGetGasPercentage(MQ7Read(MQ7_PIN)/Ro,GAS_LPG);
  iPPM_CO = MQGetGasPercentage(MQ7Read(MQ7_PIN)/Ro,GAS_CO);
  iPPM_Smoke = MQGetGasPercentage(MQ7Read(MQ7_PIN)/Ro,GAS_SMOKE);
